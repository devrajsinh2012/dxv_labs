...
void main() {
    ...
    transformed.x += sin(time + position.y * uWaveFrequency) * 0.5 * uWaveAmplitude * waveFactor;
    transformed.y += cos(time + position.z * uWaveFrequency) * 0.15 * uWaveAmplitude * waveFactor;
    transformed.z += sin(time + position.x * uWaveFrequency) * uWaveAmplitude * waveFactor;
...
class AsciiFilter {
  ...
  constructor(
    renderer: THREE.WebGLRenderer,
    { fontSize, fontFamily, charset, invert }: any = {}
  ) {
    ...
    // Disable image smoothing for crisp retro ASCII rendering
...
class CanvAscii {
  ...
  constructor(
    ...
    this.waveSpeed = waveSpeed ?? 5.0; // Default matches original 'uTime * 5.'
    this.waveFrequency = waveFrequency ?? 1.0;
    this.waveAmplitude = waveAmplitude ?? 1.0;
    ...
    >
      <style>{`
        ...
        .ascii-text-container pre {
          ...
          left: 0;
          top: 0;
          background-image: radial-gradient(circle, #ff5722 0%, #fc9867 50%, #ffd866 100%);
