  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        ...
        this.containerHeight = rect.height || 1;
      ...
      setCoords(pageX: number, pageY: number) {
        if (this.timer) window.clearTimeout(this.timer);
        const nx = (pageX - this.offsetX) / this.containerWidth;
        const ny = (pageY - this.offsetY) / this.containerHeight;
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          const nx = (event.pageX - this.offsetX) / this.containerWidth;
          const ny = (event.pageY - this.offsetY) / this.containerHeight;
...
`;
    ...
    class ShaderPass {
      ...
      init() {
        ...
        if (this.uniforms && Common.renderer) {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: any) {
        let forceX = 0;
        let forceY = 0;
        if (Mouse.isHoverInside || Mouse.isAutoActive) {
          forceX = (Mouse.diff.x / 2) * props.mouse_force;
    ...
    class WebGLManager {
      ...
      resize() {
        ...
        this.output?.resize();
        if (Mouse.container) {
          const rect = Mouse.container.getBoundingClientRect();
          Mouse.offsetX = rect.left + window.scrollX;
          Mouse.offsetY = rect.top + window.scrollY;
          Mouse.containerWidth = rect.width || 1;
          Mouse.containerHeight = rect.height || 1;
        }
      }
      render() {
